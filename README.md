...
`price` (the target) is a string column (`"$234.50"` style) and only 67.2% populated, so
the other ~30k listings have no label, can't be used for supervised training, and are
excluded rather than imputed. `log1p(price)` is close to normal and centred around 5.2
(back-transforming to ~£180, the actual median), confirming it as the right target
transform. The 99th percentile price is £1,373 against a max of £527,524: the top 0.1%
of listings are 84x the 99.9th-percentile value, so extreme values are capped at the 99th
percentile before logging rather than left to distort a linear model.
...
0.25% each). Review volume and score barely move price (all correlations under 0.15) and
...
strongest of these, since AC is genuinely rare in UK housing stock, so its presence reads
as a premium-property signal rather than a baseline expectation. One flag
...
meaningful bar, so the overall `review_scores_rating` composite is skipped in favour of
that single subscore.

**Preprocessing pipeline (Fri):** target is `log_price_capped`, computed as
...
Two notebooks: `pipeline.ipynb` (EDA, feature engineering, the preprocessing pipeline
through `X_train_transformed`) and `modelling.ipynb` (model training and comparison,
currently a skeleton, see "What Saturday adds" above). The second notebook doesn't share
kernel state with the first, so it opens with a short section that re-runs the pipeline
steps needed to rebuild `X_train_transformed`/`y_train` before the modelling cells. Run
each notebook top to bottom.

## Known limitations

- The ~30k listings with no `price` are dropped for training, not imputed. No amount of
  imputation fixes a missing target without fabricating labels, and estimating price for
...
- The Random Forest preprocessor (once built) will likely reuse the same
  ...
  scaling. That's a deliberate simplification to keep every model comparable on the same
